export type Page = 'upgrades' | 'resources' | 'food' | 'cats' | 'minigames' | 'match3' | 'pairs'
...
const pages: { id: Exclude<Page, 'match3' | 'pairs'>; name: string; icon: string }[] = [
...
export function Navigation({ page, onChange }: Props) {
  return (
    <nav className="navigation" aria-label="Разделы игры">
      {pages.map((item) => {
        const active = page === item.id || (item.id === 'minigames' && (page === 'match3' || page === 'pairs'))
