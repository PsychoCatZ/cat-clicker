...
function App() {
  ...
  const [page, setPage] = useState<Page>('upgrades')
  ...
  function resetProgress() {
    if (window.confirm('Полностью сбросить все комнаты, рыбок, котов, ресурсы и улучшения?')) {
      dispatch({ type: 'reset' })
      setPage('upgrades')
  ...
  return <main className="app-shell">
    ...
    <nav className="room-tabs" aria-label="Комнаты">
      {rooms.map((item) => <button key={item.id} type="button" disabled={item.id > state.unlockedRoom}
        ...
        onClick={() => { dispatch({ type: 'visitRoom', roomId: item.id }); setPage('upgrades') }}>
